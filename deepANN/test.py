...
a1=SDAE(numpy.random,RandomStreams(),act='tanh', depth=3,one_sided_in=True,n_hid=1000,regularization=False,sparsity=False,
            reconstruction_cost='cross_entropy',n_inp=1024,n_out=62,tie=False,bbbool = False)


#a2=SDAE(numpy.random,RandomStreams(),act='tanh', depth=2,one_sided_in=True,n_hid=500,regularization=False,sparsity=False,
            #reconstruction_cost='cross_entropy',n_inp= 784,n_out=10,tie=False,bbbool = True)
...
trainl = theano.shared(load_mat('PNIST07_train1_data.ft', path1))
...
a1.auxiliary(1, 0,1024, auxact = 'rectifier', auxwdreg = 'l2', Wtmp = None, btmp = None)
a1.ModeAux(1,update_type='global',lr=0.001)
...
a1.aux=False
a1.ModeUnsup(2,1,noise_lvl = 0.25 , update_type='global',lr=0.001)
g1,n = a1.trainfunctionbatch([train,255.,theano.config.floatX],None,None, batchsize=batchsize)
for i in range(n):
    g1(i),i


a1.auxiliary(2, 1,1024, auxact = 'rectifier', auxwdreg = 'l2', Wtmp = a1.auxlayer.W, btmp = a1.auxlayer.b)
a1.ModeAux(2,update_type='global',lr=0.001)
g1,n = a1.trainfunctionbatch([train,255.,theano.config.floatX],None,[trainl,255.,theano.config.floatX],\
                batchsize=batchsize)
for i in range(n):
    g1(i),i
...
a1.aux=False
a1.ModeUnsup(3,2,noise_lvl = 0.25 , update_type='global',lr=0.001)
g1,n = a1.trainfunctionbatch([train,255.,theano.config.floatX],None,None, batchsize=batchsize)
for i in range(n):
    g1(i),i


a1.auxiliary(3, 2,1024, auxact = 'rectifier', auxwdreg = 'l2', Wtmp = a1.auxlayer.W, btmp = a1.auxlayer.b)
a1.ModeAux(2,update_type='global',lr=0.001)
g1,n = a1.trainfunctionbatch([train,255.,theano.config.floatX],None,[trainl,255.,theano.config.floatX],\
                batchsize=batchsize)
for i in xrange(n):
...
screen = pygame.Surface((2*32,2*34*3),depth=32)
it = 0
for i in range(1000):
    screen.fill(0)
    x = train.value[i,:]
    ...
    new2 = pygame.transform.scale(new, (32*2, 32*2))
    screen.blit(new2,(0,0))
    x = trainl.value[i,:]
    im = numpy.asarray([numpy.reshape(x,(32,32))]*3).T
    new=pygame.surfarray.make_surface(im)
    new2 = pygame.transform.scale(new, (32*2, 32*2))
    screen.blit(new2,(0,2*32*2))
    x = a1.auxrepresentation(train.value[i:i+1,:])[0,:]
    im = numpy.asarray([numpy.reshape(x,(32,32))*255]*3).T
    new=pygame.surfarray.make_surface(im)
    new2 = pygame.transform.scale(new, (32*2, 32*2))
    screen.blit(new2,(0,2*32*1))
    pygame.image.save(screen,'/u/glorotxa/rectifier/%s.PNG'%i)
    raw_input('')
